...
list_of_appropriate_users = Config.list_of_appropriate_users
...
async def bot_start(message: types.Message):
    await bot.send_sticker(
        ...
        sticker="CAACAgIAAxkBAAEKBMpk2i-xj81o4YuEuPHgEa8xX2KFTwAC6QEAAll7vwN8MkD8fnPP6TAE",
    )
...
async def main(message: types.Message):
    ...
    if message.text == "Сгенерировать креативы 🤖":
        a = ""
        choose = await bot.send_message(
            chat_id=message.chat.id,
            text="Выбери категорию оффера 🙇🏻‍♂️",
        ...
        print(a)
        choose = await message.answer(
            text="Выбери размер пака 💁🏻‍♂️", reply_markup=keyboard6
        )
    elif message.text == "Классический👨🏻‍⚕️":
        a += f", {message.text[:-5]}, Стандарт (две рамки)"
        print(a)
        choose = await message.answer(
            text="Выбери формат баннера👨🏻‍🔧", reply_markup=keyboard7
        )
    ...
    # choose = await message.answer(text='Выбери размер пака 💁🏻‍♂️', reply_markup=keyboard6)
        ...
        print(a)
        choose = await message.answer(
            text="Выбери вид креатива👨🏻‍💻", reply_markup=keyboard2
        )
        ...
        await bot.send_chat_action(
            message.chat.id, action=types.ChatActions.TYPING
        )  # отправляем "типинг"
        a += ", " + message.text[0]
        print(a)
        ...
        try:
            ...
            await bot.send_message(
                message.chat.id,
                text="Выбирайте пока только существующие креативы!",
                reply_markup=keyboard1,
            )
        ...
        if message.text == "◀️Назад":
            ...
            if choose.text == "Выбери категорию оффера 🙇🏻‍♂️":
                ...
                choose = await message.answer(
                    text="Выбери язык 💆🏻‍♂️", reply_markup=keyboard5
                )
                ...
                choose = await bot.send_message(
                    message.chat.id, text="Выбери ГЕО 🌎", reply_markup=keyboard4
                )

            elif (
                choose.text == "Выбери размер пака 💁🏻‍♂️"
                and a.split(", ")[-1] == "товарные"
            ):
                choose = await message.answer(
                    text="Выбери вид креатива👨🏻‍💻", reply_markup=keyboard2
                )
        ...
        choose = await bot.send_message(
            message.chat.id,
            text="Нихуя не понял, но очень интересно!",
            reply_markup=keyboard1,
        )
...
# Так, мне нужно заняться созданием системы оплаты
# Для создания системы оплаты необходим ИП
# Видимо меня ждет настоящая ебатория, у пользователя будет выбор между оплатой через WalletPay и через Robokassa, и выплата будет привязана к долларам
# Еще нужно реализовать вебхук в моем боте
# Убрать арабов, добавить выбор 70 на 30 из другой папки 'общие', добавить 1 на 1 отзеркаливание фонов
# Полная смена папочной структуры. Каеф.
