...
def blaming_elements(background: str, frame_1: str, text_1: str, frame_2: str = None, text_2: str = None):
    """Функция для объединения переданных фрагментов. Фрагменты содержат полные пути до файлов котоыре нужно будет загрузить."""
...
def generation_elements(path_to_backgrounds: str, path_to_frames: str, path_to_texts: str, size_pack: str):
    ...
    lst = os.listdir('\\'.join(path_to_backgrounds.split('\\')[:-1])) if path_to_backgrounds.split('\\')[-1] == 'Микс' else [path_to_backgrounds.split('\\')[-1]] # Это просто список ГЕО
    shuffle(lst) # Список с ГЕО перемешали
    for i in range(first_hash[size_pack]):
        while background in second_hash:
            path = os.path.join('\\'.join(path_to_backgrounds.split('\\')[:-1]), lst[i % len(lst)]) # Путь до папки с фонами
    ...
    if path_to_frames.split('\\')[-2] == 'Фоны_две_рамки':
        for i in range(first_hash[size_pack]):
            ...
            frame_1 = os.path.join(path_to_frames, chosen_kind_of_frames, 'Верх', choice(os.listdir(os.path.join(path_to_frames, chosen_kind_of_frames, 'Верх'))))
            frame_2 = os.path.join(path_to_frames, chosen_kind_of_frames, 'Низ', choice(os.listdir(os.path.join(path_to_frames, chosen_kind_of_frames, 'Низ'))))
            ...
            while text_1_filename == text_2_filename or text_1.split('\\')[-1] == '.DS_Store' or text_2.split('\\')[-1] == '.DS_Store':
                chosen_font_of_texts = choice(os.listdir(path_to_texts))
                text_1 = os.path.join(path_to_texts, chosen_font_of_texts, 'Верх', choice(os.listdir(os.path.join(path_to_texts, chosen_font_of_texts, 'Верх')))) # Тут мы задаем путь до файла
                text_2 = os.path.join(path_to_texts, chosen_font_of_texts, 'Низ', choice(os.listdir(os.path.join(path_to_texts, chosen_font_of_texts, 'Низ'))))
    ...
    # Рамки, кажется, готовы. Теперь текста
            chosen_font_of_texts = choice(os.listdir(path_to_texts))
            text_1 = os.path.join(path_to_texts, chosen_font_of_texts, choice(os.listdir(os.path.join(path_to_texts, chosen_font_of_texts))))
            texts_1.append(text_1)
...
def zip_function(string: str):
    ...
    backgrounds, frames_1, frames_2, texts_1, texts_2 = generation_elements(backgrounds_folder, frames_folder, texts_folder, size_pack)
